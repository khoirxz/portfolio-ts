"use server";

import Image from "next/image";
import Link from "next/link";

import { getPosts } from "@/app/graphql/queries";
import Posts from "./components/posts";

const socials: { name: string; url: string; icon: React.JSX.Element }[] = [
  {
    name: "GitHub",
    url: "https://github.com/khoirxz",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="w-6 h-6"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="2"
        viewBox="0 0 24 24">
        <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.4 5.4 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65S8.93 17.38 9 18v4"></path>
        <path d="M9 18c-4.51 2-5-2-7-2"></path>
      </svg>
    ),
  },
  {
    name: "LinkedIn",
    url: "https://www.linkedin.com/in/rizqi-khoirurrohman-86a292268/",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="w-6 h-6"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="2"
        viewBox="0 0 24 24">
        <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6M2 9h4v12H2z"></path>
        <circle cx="4" cy="4" r="2"></circle>
      </svg>
    ),
  },
  {
    name: "Instagram",
    url: "https://www.instagram.com/rizqi.khoir/",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="w-6 h-6"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="2"
        viewBox="0 0 24 24">
        <rect width="20" height="20" x="2" y="2" rx="5" ry="5"></rect>
        <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37M17.5 6.5h.01"></path>
      </svg>
    ),
  },
  {
    name: "Facebook",
    url: "https://www.facebook.com/rootbackdor",
    icon: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="w-6 h-6"
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="2"
        viewBox="0 0 24 24">
        <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path>
      </svg>
    ),
  },
];

export default async function Home() {
  const data = await getPosts();

  return (
    <div className="flex flex-col items-center max-w-7xl mx-auto p-4 md:p-8">
      <section className="flex flex-col w-full gap-5 md:px-0">
        <div className="relative w-[120px] h-[120px] rounded-2xl overflow-hidden hover:scale-105 transition-all">
          <Image
            src="/profile.jpg"
            alt="Rizqi Khoirurrohman"
            fill
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
          />
        </div>

        <div>
          <h1 className="md:text-[3vw] text-[7vw] mb-4">
            Halo, saya{" "}
            <span className="text-blue-500 hover:font-[family-name:var(--font-unifraktur-maguntia)] transition-all">
              Rizqi Khoirurrohman.
            </span>{" "}
            <br />
            Saya Pengembang perangkat lunak berbasis web.
          </h1>
        </div>

        <div className="flex flex-row gap-6">
          {socials.map((social) => (
            <Link
              key={social.name}
              href={social.url}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 mb-2 hover:text-blue-500 transition-all">
              {social.icon}
            </Link>
          ))}
        </div>
      </section>

      <section className="flex flex-col w-full my-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {data.allPosts.edges.map((item) => (
            <Posts key={item.node.id} item={item} />
          ))}
        </div>
      </section>
    </div>
  );
}
